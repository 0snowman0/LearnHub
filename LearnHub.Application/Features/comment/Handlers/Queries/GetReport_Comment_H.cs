...
{
    public class GetReport_Comment_H : IRequestHandler<GetReport_Comment_R, BaseCommandResponse>
    {
        private readonly IComment _comment;
        private readonly IMapper _mapper;

        public GetReport_Comment_H(IComment comment, IMapper mapper)
        {
            _comment = comment;
            _mapper = mapper;
        }

        public async Task<BaseCommandResponse> Handle(GetReport_Comment_R request, CancellationToken cancellationToken)
        {
            var response = new BaseCommandResponse();

            var Comments = await _comment.GetReportComments();
